...
#define A3(x) _Generic((x), \
    double: ARG_DOUBLE, \
    int:    ARG_INT  \
    )

int A2(double x) { return ARG_DOUBLE; }
int A2(int x) { return ARG_INT; }
...
string format(Arena *arena, string fmt, size_t arg_count, ...) {
    ...
    size_t arg_index = 0;
    while (p < end) {
        const char *open_brace = (const char*)memchr(p, '{', end - p);
        if (open_brace == NULL) {
            string remaining = {.str = (char*)p, .size = (uint64_t)(end - p)};
            result = str_concat(arena, result, remaining);
            break;
        }
        if (open_brace > p) {
            string part = {.str = (char*)p, .size = (uint64_t)(open_brace - p)};
        ...
        const char *close_brace = (const char*)memchr(p, '}', end - p);
        if (close_brace == NULL) {
            string error = str_lit("Error: missing closing brace");
            result = str_concat(arena, result, error);
            break;
        }
        const char *colon = (const char*)memchr(p, ':', close_brace - p);
        FormatSpec spec;
        if (colon) {
            string spec_str = {.str = (char*)colon + 1,
                .size = (uint64_t)(close_brace - (colon + 1))};
            spec = parse_format_spec(spec_str);
        } else {
            if (p != close_brace) {
                string error = str_lit("Error: invalid format specifier");
                result = str_concat(arena, result, error);
                p = close_brace + 1;
                continue;
            }
        ...
        if (arg_index >= arg_count) {
            string error = str_lit("Error: missing argument");
            result = str_concat(arena, result, error);
            p = close_brace + 1;
            continue;
        }
...
int main() {
    ...
    //string result = format(arena, fmt, 1, ARG_INT, 5);
    string result = format(arena, fmt, 1, A2(5), 5);
