...
MLIR_OpHandle mlir_wasmssa_to_llvm(MLIR_Context *ctx, MLIR_OpHandle ssa_module) {    MLIR_RegionHandle mr = MLIR_GetOpRegion(ssa_module, 0);
    ...
    for (size_t i = 0; i < nops; i++) {
        ...
        if (nm.size == 7 && memcmp(nm.str, "fd_seek", 7) == 0) continue;
        if (nm.size == 7 && memcmp(nm.str, "fd_tell", 7) == 0) continue;
        if (nm.size == 8 && memcmp(nm.str, "fd_close", 8) == 0) continue;
        if (nm.size == 9 && memcmp(nm.str, "path_open", 9) == 0) continue;
        if (nm.size == 8 && memcmp(nm.str, "args_get", 8) == 0) continue;
        if (nm.size == 14 && memcmp(nm.str, "args_sizes_get", 14) == 0) continue;
        if (nm.size == 11 && memcmp(nm.str, "environ_get", 11) == 0) continue;
        if (nm.size == 17 && memcmp(nm.str, "environ_sizes_get", 17) == 0) continue;
        fprintf(stderr,
    ...
    // recognises them so they are accepted as imports. va_arg is lowered inline
    // by tinyC's emit.c, so the lifted module never references the old
    // tinyc_va_arg_* helpers either.
